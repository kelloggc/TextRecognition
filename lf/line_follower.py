...
class LineFollower(nn.Module):
    def __init__(self, output_grid_size=32, dtype=torch.FloatTensor):
    ...
    def forward(self, image, positions, steps=None, all_positions=[], reset_interval=-1, randomize=False, negate_lw=False, skip_grid=False, allow_end_early=False):
        ...
        view_window = Variable(torch.FloatTensor([
            [2,0,2],
            [0,2,0],
            [0,0,1]
        ])).expand(batch_size,3,3)

        step_bias = Variable(torch.FloatTensor([
            [1,0,2],
            [0,1,0],
            [0,0,1]
        ])).expand(batch_size,3,3)

        invert = Variable(torch.FloatTensor([
            [-1,0,0],
            [0,-1,0],
            [0,0,1]
        ])).expand(batch_size,3,3)
